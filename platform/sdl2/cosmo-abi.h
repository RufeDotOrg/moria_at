int abi_SDL_SetError(SDL_PRINTF_FORMAT_STRING const char *a, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_SetError");
    if (!IsWindows()) {
        int (*local)(SDL_PRINTF_FORMAT_STRING const char *a, ...) = addr;
        return local(a);
    } else {
        int __attribute__((__ms_abi__)) (*local)(SDL_PRINTF_FORMAT_STRING const char *a, ...) = addr;
        return local(a);
    }
}
void abi_SDL_Log(SDL_PRINTF_FORMAT_STRING const char *a, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_Log");
    if (!IsWindows()) {
        void (*local)(SDL_PRINTF_FORMAT_STRING const char *a, ...) = addr;
        local(a);
    } else {
        void __attribute__((__ms_abi__)) (*local)(SDL_PRINTF_FORMAT_STRING const char *a, ...) = addr;
        local(a);
    }
}
void abi_SDL_LogVerbose(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_LogVerbose");
    if (!IsWindows()) {
        void (*local)(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        local(a, b);
    } else {
        void __attribute__((__ms_abi__)) (*local)(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        local(a, b);
    }
}
void abi_SDL_LogDebug(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_LogDebug");
    if (!IsWindows()) {
        void (*local)(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        local(a, b);
    } else {
...
void abi_SDL_LogInfo(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_LogInfo");
    if (!IsWindows()) {
        void (*local)(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        local(a, b);
    } else {
...
void abi_SDL_LogWarn(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_LogWarn");
    if (!IsWindows()) {
        void (*local)(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        local(a, b);
    } else {
...
void abi_SDL_LogError(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_LogError");
    if (!IsWindows()) {
        void (*local)(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        local(a, b);
    } else {
...
void abi_SDL_LogCritical(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_LogCritical");
    if (!IsWindows()) {
        void (*local)(int a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        local(a, b);
    } else {
...
void abi_SDL_LogMessage(int a, SDL_LogPriority b, SDL_PRINTF_FORMAT_STRING const char *c, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_LogMessage");
    if (!IsWindows()) {
        void (*local)(int a, SDL_LogPriority b, SDL_PRINTF_FORMAT_STRING const char *c, ...) = addr;
        local(a, b, c);
    } else {
        void __attribute__((__ms_abi__)) (*local)(int a, SDL_LogPriority b, SDL_PRINTF_FORMAT_STRING const char *c, ...) = addr;
        local(a, b, c);
    }
}
int abi_SDL_sscanf(const char *a, SDL_SCANF_FORMAT_STRING const char *b, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_sscanf");
    if (!IsWindows()) {
        int (*local)(const char *a, SDL_SCANF_FORMAT_STRING const char *b, ...) = addr;
        return local(a, b);
    } else {
        int __attribute__((__ms_abi__)) (*local)(const char *a, SDL_SCANF_FORMAT_STRING const char *b, ...) = addr;
        return local(a, b);
    }
}
int abi_SDL_snprintf(SDL_OUT_Z_CAP(b) char *a, size_t b, SDL_PRINTF_FORMAT_STRING const char *c, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_snprintf");
    if (!IsWindows()) {
        int (*local)(SDL_OUT_Z_CAP(b) char *a, size_t b, SDL_PRINTF_FORMAT_STRING const char *c, ...) = addr;
        return local(a, b, c);
    } else {
        int __attribute__((__ms_abi__)) (*local)(SDL_OUT_Z_CAP(b) char *a, size_t b, SDL_PRINTF_FORMAT_STRING const char *c, ...) = addr;
        return local(a, b, c);
    }
}
...
int abi_SDL_Vulkan_LoadLibrary(const char *a)
{
    void *addr = cosmo_dlsym(libD, "SDL_Vulkan_LoadLibrary");
    if (!IsWindows()) {
        int (*local)(const char *a) = addr;
        return local(a);
    } else {
...
void *abi_SDL_Vulkan_GetVkGetInstanceProcAddr(void)
{
    void *addr = cosmo_dlsym(libD, "SDL_Vulkan_GetVkGetInstanceProcAddr");
    if (!IsWindows()) {
        void *(*local)(void) = addr;
        return local();
    } else {
...
void abi_SDL_Vulkan_UnloadLibrary(void)
{
    void *addr = cosmo_dlsym(libD, "SDL_Vulkan_UnloadLibrary");
    if (!IsWindows()) {
        void (*local)(void) = addr;
        local();
    } else {
...
SDL_bool abi_SDL_Vulkan_GetInstanceExtensions(SDL_Window *a, unsigned int *b, const char **c)
{
    void *addr = cosmo_dlsym(libD, "SDL_Vulkan_GetInstanceExtensions");
    if (!IsWindows()) {
        SDL_bool (*local)(SDL_Window * a, unsigned int *b, const char **c) = addr;
        return local(a, b, c);
    } else {
        SDL_bool __attribute__((__ms_abi__)) (*local)(SDL_Window * a, unsigned int *b, const char **c) = addr;
        return local(a, b, c);
    }
}
...
int abi_SDL_asprintf(char **a, SDL_PRINTF_FORMAT_STRING const char *b, ...)
{
    void *addr = cosmo_dlsym(libD, "SDL_asprintf");
    if (!IsWindows()) {
        int (*local)(char **a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        return local(a, b);
    } else {
        int __attribute__((__ms_abi__)) (*local)(char **a, SDL_PRINTF_FORMAT_STRING const char *b, ...) = addr;
        return local(a, b);
    }
}
